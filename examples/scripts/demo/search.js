...
TrackSearchDemo.prototype.Setup = function() {
...
TrackSearchDemo.prototype.RunSearch = function(query, pagenum) {
    var settings = this.settings,
    ...
    $.get(settings.apiUrl + query + "&page=" + pagenum, function (data) {
        if (data.status == "ok") {
            _.each(data.searchResults.searchResult, function (searchResult) {
                settings.displayElement.append(template(searchResult.track));
            }, this);
...
TrackSearchDemo.prototype.LoadNext = function() {
    var settings = this.settings;
...
var trackSearchDemo = new TrackSearchDemo().Setup().RunSearch("", 1);

$(window).scroll(function () {
    if ($(window).scrollTop() + $(window).height() == $(document).height()) {
        trackSearchDemo.LoadNext();
    }
});